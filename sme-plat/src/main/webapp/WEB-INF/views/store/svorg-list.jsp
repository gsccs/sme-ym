...
<div id="svorg_window"></div>
	<script type="text/javascript">
		$('#list_data').datagrid({
			...
			fit : true,//自动大小 
			loadMsg : '数据加载中请稍后……',
			url : '${pageContext.request.contextPath}/svorg/datagrid',
			remoteSort : false,
			fitColums : true,
			checkOnSelect : true,
		...
		function edit(){
			...
			if (checkedRows.length == 1)
				self.location = "${pageContext.request.contextPath}/svorg/edit?id="
						+ checkedRows[0].id;
		...
		//新增
		 function add(){
			 self.location = "${pageContext.request.contextPath}/svorg/add";
		} 
