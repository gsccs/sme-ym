...
<script type="text/javascript">
	...
	<fieldset>
		<legend style="font-weight: bold; font-size: 16pt; color: #E3393C;">服务需求编辑</legend>
		<form id="myform" action="edit" method="post">
			<input type="hidden" name="id" value="${svorgT.id}" />
			<table cellpadding="3" cellspacing="0" class="edit_table"
				...
				<tr>
					...
					<c:choose>
						<c:when test="${svorgT.naturestr.contains(listSOBJ.title)}">
							<input type="radio" name="nature" id="${listSOBJ.id}"
								value="${listSOBJ.id}" checked="checked" />
				...
				<tr>
					...
					<td><select name="scode" id="scode" onchange="a(this.value)"
						style="width: 190px;">
							...
							<option value="${svorgT.subscode}">${svorgT.subsclassstr}</option>
					</select></td>
				</tr>
				<tr>
				...
				<tr style="height: 50px;">
					<td colspan="2" style="text-align: right"><a id="btnAdd"
						...
						href="javascript:" onclick="submit()">确认修改</a> <a id="btnBack"
