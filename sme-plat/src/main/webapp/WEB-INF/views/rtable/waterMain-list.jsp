 ...
 <form id="searchForm">
	...
	<table id="list_data" cellspacing="0" cellpadding="0">
		<thead>
			<tr>
				...
				<th field="tjdatestr" width="200">上报时间</th>
			</tr>
		</thead>
	</table>
	</div>
	...
	<script type="text/javascript">
		$('#list_data').datagrid({
			...
			fit : true,//自动大小 
			loadMsg : '数据加载中请稍后……',
			url : '${pageContext.request.contextPath}/waterMain/datagrid',
			remoteSort : false,
			fitColums : true,
			checkOnSelect : true,
		...
		function edit(){
			 ...
			 if(checkedRows.length == 1) {
				 self.location = "${pageContext.request.contextPath}/waterMain/info?mainId="
		...
		//新增
		 function add(){
			 self.location = "${pageContext.request.contextPath}/waterMain/add";
		}
		//删除
		function del(){
			var checkedRows = $("#list_data").datagrid('getChecked');
			if(checkedRows.length == 1) {
				$.messager.confirm('确认', '您是否要删除当前选中的记录？', function(r) {
					if(r) {
						$.ajax({
							url : '${pageContext.request.contextPath}/waterMain/delete',
							data : {
