...
<body>
	<script type="text/javascript">
		...
		$(function() {
			...
			$("#saveTopicBtn").click(function() {
				var id = $("#reportid").val();
					url = ctx + "/reportitem/addbyattach";
				
				var postData = {
					reportid : $("#reportid").val(),
					submitdate : $("#submitdate").val(),
					remark : $("#remark").val(),
					isattach : $("#isattach").val(),
					attachs : attachs
				};
	...
	<fieldset>
		<legend style="font-weight: bold; font-size: 16pt; color: #E3393C;">报表信息</legend>
		<form>
			<table class="tableForm" style="margin-top: 10px;">
				<tr class="thclass" style="height: 40px">
					<th style="width: 130px; font-size: 14px;">报表名称<font
						style="color: red">*</font></th>
					<td><label 	style="width: 300px; height: 30px" >${report.title}</label></td>
				</tr>

				<tr class="thclass" style="height: 40px">
					<th style="width: 130px; font-size: 14px;">报送周期<font
						style="color: red">*</font></th>
					<td> <label><c:if test="${report.cyc=='D' }"> 日 </c:if></label>
						 <label><c:if test="${report.cyc=='W' }"> 周 </c:if></label>
						 <label><c:if test="${report.cyc=='M' }"> 月 </c:if></label>
						 <label><c:if test="${report.cyc=='Q' }"> 季度 </c:if></label>
						 <label><c:if test="${report.cyc=='Y' }"> 年 </c:if></label>
					</td>
				</tr>
				...
				<tr class="thclass" style="height: 40px">
					<th style="width: 130px; font-size: 14px;">备注说明<font
						style="color: red">*</font></th>
					<td><label 	style="width: 345px; height: 100px;">${report.remark}</label></td>
				</tr>
				<tr class="thclass" style="height: 40px">
					...
					<td>
						<ul style="padding-left: 0px;">
							<c:forEach items="${report.attachs}" var="attach">
								...
								<a href="${attach.filepath}">${attach.filename}</a>
								</li>
							</c:forEach>
						</ul>
					</td>
	...
	<fieldset>
		<legend style="font-weight: bold; font-size: 16pt; color: #E3393C;">上报信息</legend>
		<form id="reportform" method="post">
			<input type="hidden" id="reportid" name="id" value="${report.id}" />
			<input type="hidden" id="isattach" name="isattach" value="${report.isattach}" />
			<table class="tableForm" style="margin-top: 10px;">
				<tr class="thclass" style="height: 40px">
					<th style="width: 130px; font-size: 14px;">报出日期<font
						style="color: red">*</font></th>
					<td><input style="width: 100px;" name="submitdate" id="submitdate" value="new Date()"
				...
				<tr class="thclass" style="height: 40px">
					...
					<td><textarea name="remark" id="remark"
							...
							style="width: 345px; height: 100px;"></textarea></td>
				</tr>
				...
				<tr class="thclass" style="height: 40px">
					...
					<td>
						<ul id="filelist" style="padding-left: 0px;">
							<c:forEach items="${reportItemT.attachs}" var="attach">
