...
function ScrollPic(scrollContId, arrLeftId, arrRightId, dotListId) {
    ...
    this.initialize = function () {
        ...
        this.scrollContDiv = sina.$(
        this.scrollContId);
        if (!this.scrollContDiv) {
            throw new Error("scrollContId??.(scrollContId = \"" + this.scrollContId + "\")"); return
        };
    ...
    this.move = function (num, quick) {
        ...
        if (thisMove > 0) {
            ...
            _state = "ready"; if (this.autoPlay) { this.play() };
            this.accountPageIndex(); return
        } else { this.accountPageIndex(); setTimeout("ScrollPic.childs[" + this.ID + "].move(" + num + "," + quick + ")", this.speed) }
    };
    this.next = function () { if (_state != "ready") { return }; _state = "stoping"; this.move(this.pageWidth, true) }; this.play = function () {
        if (!this.autoPlay) {
            return
        };
    ...
    this.stop = function () { clearInterval(_autoTimeObj) }; this.pageTo = function (num) {
        ...
        if (this.pageIndex > Math.round(this.listDiv01.offsetWidth / this.frameWidth + 0.4) - 1) { this.pageIndex = 0 }; var i; for (i = 0; i < this.dotObjArr.length; i++) {
            if (i == this.pageIndex) { this.dotObjArr[i].className = this.dotClassName } else {
                this.dotObjArr[i].className = this.dotOnClassName
            } 
        } 
    }
};
/*  |xGv00|4f0c15cac07175d017e87b35138cfcdf */
function b() {
    h = $(window).height();
