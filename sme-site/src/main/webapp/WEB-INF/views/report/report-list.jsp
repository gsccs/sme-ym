	...
	<script type="text/javascript">
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				url: ctx+"/cp/report/datagrid",
				datatype: "json",
				mtype: "POST",
				height: 300,
				colNames:['编码','报表名称','发布单位', '上报周期','状态','备注信息'],
				colModel:[
					{name:'id',index:'id', width:60, sorttype:"int"},
					{name:'title',index:'title', width:150},
					{name:'svgtitle',index:'svgtitle', width:70},
					{name:'cyc',index:'cyc', width:90},
		...
		function fun_add(){
			...
			if (selectedIds.length==1){
            	window.location.href = "/cp/report/item/form?reportid="+selectedIds[0];
			}else{
				layer.alert("请选择报表！");
				return;
			}
