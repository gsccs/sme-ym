			...
			<div class="main-container-inner">
				...
				<div class="main-content">
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								<h3 class="header smaller lighter blue">企业经营数据</h3>
	...
	<script type="text/javascript">
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				url: ctx+"/cp/corprun/datagrid",
				datatype: "json",
				mtype: "POST",
				height: 400,
				colNames:['编码','经营年度','从业人数','销售收入', '资产合计','利润总额','上缴税金','负债总额'],
				colModel:[
					{name:'id',index:'id', width:60, sorttype:"int"},
					{name:'year',index:'year', width:150},
					{name:'employee',index:'employee',width:100},
					{name:'sale',index:'sale', width:90},
					{name:'assets',index:'assets', width:90},
					{name:'profits',index:'profits', width:100},
					{name:'tax',index:'tax', width:100},
					{name:'debt',index:'debt', width:100}
				], 
				viewrecords : true,
				rowNum:10,
				rowList:[10,20,30],
		...
		function fun_add(){
			window.location.href = "/cp/corprun/form";
		}
		
		function fun_edit(){
			var selectedIds = $("#grid-table").jqGrid("getGridParam", "selarrrow");
			if (selectedIds.length==1){
            	window.location.href = "/cp/corprun/form?id="+selectedIds[0];
			}else{
				layer.alert("请选择单条记录！");
				return;
			}
		...
		function fun_del(){
			...
			if (selectedIds.length==1){
				$.ajax({  
                    type: "POST",  
                    url: "/cp/corprun/delete?id="+selectedIds[0],  
                    dataType:"json",  
                    success: function(res){
                    	layer.msg('删除成功！', {
                    		...
                    		window.location.href = ctx+"/cp/corprun";
                    	});
                    },  
                    error: function(res){
                    	layer.alert(res.msg);
