	...
	<script type="text/javascript">
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				url: ctx+"/cp/declare/topic/datagrid",
				datatype: "json",
				mtype: "POST",
				height: 300,
				colNames:['编码','项目名称','项目类型','受理单位', '开始时间','结束时间','状态','备注信息'],
				colModel:[
					{name:'id',index:'id', width:60},
					{name:'title',index:'title',width:200},
					{name:'dectypestr',index:'dectypestr',width:100},
					{name:'svgtitle',index:'svgtitle', width:170},
		...
		function fun_add(){
			...
			if (selectedIds.length==1){
            	window.location.href = "/cp/declare/form?topicid="+selectedIds[0];
			}else{
				layer.alert("请选择申报项目记录！");
				return;
			}
		}
