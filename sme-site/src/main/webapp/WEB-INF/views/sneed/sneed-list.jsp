	...
	<script type="text/javascript">
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				url: ctx+"/cp/sneed/datagrid",
				datatype: "json",
				mtype: "POST",
				height: 300,
				colNames:['需求编码','需求名称','需求分类',  '提交时间','状态','备注信息'],
				colModel:[
		...
		function fun_view(){
			...
			if (selectedIds.length==1){
            	window.location.href = "/cp/sneed/view?id="+selectedIds[0];
			}else{
				layer.alert("请选择单条记录！");
				return;
			}
		...
		function fun_cancel(){
			var selectedIds = $("#grid-table").jqGrid("getGridParam", "selarrrow");
			if (selectedIds.length==1){
				$.post(ctx+"/cp/sneed/cancel?id="+selectedIds[0], function (data) {
	            	console.log("sclassList",data);
	            });
            	window.location.href = "/cp/consult/view?id=${sneed.id}";
			}else{
				layer.alert("请选择单条记录！");
				return;
			}
